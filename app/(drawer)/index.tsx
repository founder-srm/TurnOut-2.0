...
export default function Home() {
  ...
  const scanQRCodeFromImage = async (uri: string) => {
    if (Platform.OS !== "web") {
      Alert.alert("Feature not available", "QR scanning from gallery is only available on web platform")
      setImage(null)
      return
    }
      ...
      return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
          ...
          if (qrCode) {
            ...
            Alert.alert("No QR Code Found", "Please try another image or scan directly with camera")
  ...
  const markAttendance = async (qrData: string) => {
    ...
    try {
      ...
      if (checkData.attendance !== "Present") {
        ...
        const { error: altError } = await supabase.rpc('mark_attendance', { registration_id: qrData })
  ...
  if (!permission?.granted) {
    return (
      <View className="flex-1 items-center justify-center">
        <Text className="text-center mb-4">Camera permission is required</Text>
        <TouchableOpacity onPress={requestPermission} className="bg-yellow-400 px-6 py-3 rounded-lg">
      ...
      {image && (
        <View className="flex h-full w-full items-center justify-center">
          ...
          <TouchableOpacity
            ...
            disabled={loading}
          >
            <Image source={closeImg} style={{ height: 50, width: 50, opacity: loading ? 0.5 : 1 }} />
          </TouchableOpacity>
        </View>
      )}
    </View>
